    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        for step in range(self.episode_length):
            ...
            if warmup:
                ...
                acts_batch, _ = policy.get_actions(obs_batch, t_env=self.total_env_steps, explore=explore)
        ...
        for step in range(self.episode_length):
            for agent_id, p_id in zip(self.agent_ids, self.policy_ids):
                ...
                if warmup:
                    ...
                    # get actions with exploration noise (eps-greedy/Gaussian)
                    act, _ = policy.get_actions(agent_obs[agent_id], t_env=self.total_env_steps, explore=explore)
