...
def get_config():
    parser = argparse.ArgumentParser(description="OFF-POLICY", formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument("--n_rollout_threads", type=int, default=1, help="Number of parallel envs for training rollout")
    parser.add_argument(
        "--n_eval_rollout_threads", type=int, default=1, help="Number of parallel envs for evaluating rollout"
    )
    parser.add_argument("--num_env_steps", type=int, default=2000000, help="Number of env steps to train for")
    parser.add_argument(
        "--use_wandb",
        action="store_false",
        default=True,
    ...
    parser.add_argument(
        "--use_popart", action="store_true", default=False, help="Whether to use popart to normalize the target loss"
    )
    parser.add_argument(
        "--popart_update_interval_step", type=int, default=2, help="After how many train steps popart should be updated"
    )
    ...
    parser.add_argument("--per_eps", type=float, default=1e-6, help="Eps term for prioritized experience replay")
    parser.add_argument(
        "--per_beta_start", type=float, default=0.4, help="Starting beta term for prioritized experience replay"
    )

    # network parameters
    parser.add_argument(
        "--use_centralized_Q", action="store_false", default=True, help="Whether to use centralized Q function"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--data_chunk_length", type=int, default=80, help="Time length of chunks used to train via BPTT"
    )
    parser.add_argument(
        "--burn_in_time", type=int, default=0, help="Length of burn in time for RNN training, see R2D2 paper"
    )
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="Number of buffer transitions to train on at once")
    ...
    parser.add_argument(
        "--use_huber_loss", action="store_true", default=False, help="Whether to use Huber loss for critic update"
    )
    parser.add_argument("--huber_delta", type=float, default=10.0)

    # soft update parameters
    parser.add_argument("--use_soft_update", action="store_false", default=True, help="Whether to use soft update")
    ...
    parser.add_argument("--target_entropy_coef", type=float, default=0.5, help="Initial temperature")
    parser.add_argument(
        "--automatic_entropy_tune", action="store_false", default=True, help="Whether use a centralized critic"
    )
    # qmix parameters
    parser.add_argument("--use_double_q", action="store_false", default=True, help="Whether to use double q learning")
    parser.add_argument(
        "--hypernet_layers", type=int, default=2, help="Number of layers for hypernetworks. Must be either 1 or 2"
    )
    parser.add_argument("--mixer_hidden_dim", type=int, default=32, help="Dimension of hidden layer of mixing network")
    parser.add_argument(
        "--hypernet_hidden_dim",
        type=int,
        default=64,
    ...
    parser.add_argument(
        "--epsilon_start", type=float, default=1.0, help="Starting value for epsilon, for eps-greedy exploration"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_interval", type=int, default=100, help="Number of episodes between updates to actor/critic"
    )
    parser.add_argument("--use_value_active_masks", action="store_true", default=False)

    # eval parameters
    parser.add_argument("--use_eval", action="store_false", default=True, help="Whether to conduct the evaluation")
    parser.add_argument(
        "--eval_interval", type=int, default=10000, help="After how many episodes the policy should be evaled"
    )
    parser.add_argument("--num_eval_episodes", type=int, default=32, help="How many episodes to collect for each eval")
